...
struct U {                                          //|
	string name;                                   //|
	string word;
	int x,y,blood,kill,speed,up_blood,hui;
	int fan;                      //|
	int money;
	int Head,Boodly,Foot,xihd;
	string headname,boodlyname,foorname;
	int xntime,xnmax,xnmin; 
};
bool SHIXUE;
int dx[4]= {0,0,1,-1};
int dy[4]= {1,-1,0,0};
...
struct bullet {
	string ch;
	int lengh,fan,maxn,x,y,di,kill;
	Timeh t;
	void clear() {
		ch="\0";
		lengh=0;
...
struct bio {
	...
	void move() {
		...
		if(nextf==LEFT) y--;
		if(nextf==RIGHT) y++;
		if(lei==1&&x==User.x&&y==User.y/2) {
			if(User.xihd<kill) User.blood=User.blood-(kill-User.xihd);	
			if(User.xihd>0) User.xihd--;
		}
		gotoxy(y*2,x);
		cout<<ot;
		shi[x][y]=true;
...
struct weapon {
	int kill,up_bullet,now_bullet,speed,bspeed,lei;
...
void Get_New_Bio(int lei,int x,int y) {
	...
	zombie[zbnum].kill=zombie[zbnum].kill*(100-User.Boodly-User.Foot-User.Head)/100;
}
void about_us() {
	system("cls");
	Color(15);
...
void shop() {
	...
	Button A4 = NewButton(9,3,3,4,"[轻便型密合衣] $700");
	Color(15);
	system("cls");
	A1.out();
	A2.out();
	...
	while(1) {
		...
		if(A4.Preserve()&&User.money>=700&&f) QBX=true,User.money-=700,buy.last_time=clock(),f=false;
...
void BFCD() {
	...
	string the_road;
	cin>>the_road;
	if(the_road=="lcb90211245495327getmoney&&") {
	    User.money+=1000;
	    menu();
	}
...
void menu() {
	...
	Button A7 = NewButton(17,3,3,4,"[Exit Game]");
	gotoxy(0,5);
	printf("◎              ");
	gotoxy(28,5);
	printf("◎\n");
...
//			if(ifmusic)system("start music3.mp3");//
		...
		if(A7.Preserve()) {
			if(ifmusic) system("start music4.mp3");
			exit(0);
		}
	}
}
...
void how_to_ply() {
	...
	printf("操作方法：");
	gotoxy(2,3);
	printf("wasd移动方向键/鼠标射击");
	gotoxy(2,4);
	printf("r换弹m缩小p暂停o继续");
	gotoxy(2,6);
	printf("通关后进入传送门来进入下一关");
	gotoxy(2,7);
	printf("前往lookcatbox.github.io");
	gotoxy(2,8);
	printf("获取最新版本");
	gotoxy(2,9);
	printf("QQ联系1245495327");
	gotoxy(2,10);
	printf("常见BUG及解答");
	gotoxy(2,11);
	printf("无法移动:请调整输入法");
    ...
    gotoxy(2,13);
	printf("BGM错误:请不要再BGM打开过程");
	gotoxy(2,14);
	printf("中将窗口关闭,程序会缩小窗口");
	while(1) if(A1.Preserve()) {
			menu() ;
			return;
		}
			...
			if(d[i].t.Get_time()>=50) {
				...
				if(d[i].fan==UP||d[i].fan==DOWN) {
					...
					if(User.x==d[i].x&&User.y==d[i].y) {
						if(User.xihd<d[i].kill) User.blood=User.blood-(d[i].kill-User.xihd);
						if(User.xihd>0) User.xihd--;
						vis[i]=false;
						d[i].clear();
						break;
...
void MouseFan() { //获得鼠标相对于屏幕的位置
	...
	pt.y-=User.x,pt.x-=User.y;
	if(pt.y>pt.x&&pt.y>-pt.x) mousefan=DOWN;
	if(pt.y<pt.x&&pt.y>-pt.x) mousefan=RIGHT;
    if(pt.y<pt.x&&pt.y<-pt.x) mousefan=UP;
    if(pt.y>pt.x&&pt.y<-pt.x) mousefan=LEFT;
}
void GET_bullet(int lei,int num) {
	int fanxiang;
	if(lei==1) {
...
void show_blood() {
	...
	cout<<"            ";
	gotoxy(32,5);
	cout<<"头部:"<<User.headname;
	gotoxy(32,6);
	cout<<"            ";
	gotoxy(32,6);
	cout<<"身体:"<<User.boodlyname;
	gotoxy(32,7);
	cout<<"            ";
	gotoxy(32,7);
	cout<<"脚部:"<<User.foorname;
	gotoxy(32,8);
	cout<<"            ";
	gotoxy(32,8);
	cout<<"防御:"<<User.Boodly+User.Foot+User.Head<<"%";
	gotoxy(32,9);
	cout<<"            ";
	gotoxy(32,9);
	cout<<"虚拟护盾:"<<User.xihd; 
	...
	cout<<"            ";
	cout<<'[';
	if(now_weapon.lei==2&&now_weapon.leiji>=(now_weapon.bup/3)) cout<<"■";
	else cout<<"  ";
	if(now_weapon.lei==2&&now_weapon.leiji>=(now_weapon.bup/3*2)) cout<<"■";
	else cout<<"  ";
	if(now_weapon.lei==2&&now_weapon.leiji==now_weapon.bup) cout<<"■";
	else cout<<"  ";
	cout<<']';
}
void ifzbdie() {
...
kkll:
	...
	while(1) {
		...
		if(t.Get_time()>=200) {
			t.replace();
			show_blood();
			if(GetKeyState('E')<0&&now_weapon.lei==2) {
                if(now_weapon.leiji<now_weapon.bup) now_weapon.leiji++;
		    } 
			if(all_die()) {
...
//    int tttkkktmp=10;
	...
	while(1) {
		if(User.y==14&&User.x==9) {
			...
			while(1) {
				if(A1.Preserve()) {
				    ...
				    now_weapon.leiji=0;
				    now_weapon.name="Ecu";
				    now_weapon.now_bullet=100;
				    now_weapon.ot1="[][][]#####=-";
				    now_weapon.ot2="[/^^^^`````";
				    now_weapon.bspeed=1000;
				    now_weapon.up_bullet=100;
					system("cls");
					goto kkll;
				}
		...
		if(huit.Get_time()>=1000) {
			User.blood+=User.hui;
			if(User.blood>User.up_blood*0.8) User.blood=User.up_blood;
			huit.replace();
		}
		if(t.Get_time()>=200) {
			if(GetKeyState('E')<0&&now_weapon.lei==2) {
...
//BOOL PSSP(const char *str) {
//	return PlaySound(((string)(str) + (string)(".wav")).c_str(),NULL, SND_FILENAME | SND_ASYNC | SND_NOSTOP);
//}
...
//	} 
	system("cls");
	if(ifmusic) system("start music1.mp3"); 
...
A:
	...
	if(tmp1 == "New"/*创建新账户*/) {
		...
		say("\n    Welcome "+tmp1,100);
		jz();
		if(ifmusic) system("start music2.mp3");
		menu();
	} else {
		gotoxy(14,7);
		cin>>tmp2;
		...
		say("\n     Wrong User Name or Password!",100);
		goto A;
	}
